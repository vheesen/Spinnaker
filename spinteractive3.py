import os, sys, numpy, time, ttk, traceback, tkMessageBox, matplotlib, itertools, collections, subprocess, operator
...
global variables
variables = ['def_cri','def_v0','def_hb1','def_hb2','def_b0','def_b1','def_Z1','def_hv','def_gmode','def_D0','def_mu','def_modestr','def_ablossesstr','def_velfield','def_nu1','def_nu2','def_nu3','def_nu4','def_norm','def_epsilon']

juststarted = True

def quitit():
	pars = [def_cri,def_v0,def_hb1,def_hb2,def_b0,def_b1,def_Z1,def_hv,def_gmode.get(),def_D0,def_mu,def_modestr.get(),def_ablossesstr.get(),def_velfield.get(),frequency1field.get(),frequency2field.get(),frequency3field.get(),frequency4field.get(),def_norm.get(),def_epsilon.get()]
	numpy.savetxt('pars.last',pars,fmt='%s')
	sys.exit()

def saveplotpars(fname):
	pars = [["Gamma:",def_cri],["b0:",def_b0],["b1:",def_b1],["hb1:",def_hb1],["hb2:",def_hb2],["Z1:",def_Z1],["galaxy mode:",def_gmode.get()],["mode:",def_modestr.get()],["v0:",def_v0],["hv:",def_hv],["abiatic losses:",def_ablossesstr.get()],["velocity field:",def_velfield.get()],["D0:",def_D0],["mu:",def_mu],["freq 1:",frequency1field.get()],["freq 2:",frequency2field.get()],["freq 3:",frequency3field.get()],["freq 4:",frequency4field.get()],['normalize:',def_norm.get()],['epsilon:',def_epsilon.get()],["RMS chi: ",get_global_chi()]]
...
#	global def_velfield
	...
	stuff[15] = 'RMS '+r'$\chi^2$: ' + str(round(get_global_chi(),4))
	l1 = ax0.legend(stuff,loc=10,handlelength=0, handletextpad=0,markerscale=0,fontsize=22)
...
def onclick(event):
#		self.xcoor.set('x='+str(round(event.xdata)))
#		self.ycoor.set('y='+str(round(event.ydata,3)))
#		print event.xdata
#		print event.ydata
...
def initial_plot():
	global simplot_i1, simplot_i2, simplot_i3, simplot_i4, simplot_a12, simplot_a23, simplot_a34, chi_array, spi1, spi2, spi3, data, plotfig, show_spin_out, debug_out, chi_array, chiplot_1,  chiplot_2, chiplot_3, chiplot_4, chiplot_5, chiplot_6, chiplot_7,def_norm, data
	...
	for i in range(len(plotfig.axes)):
		plotfig.axes[i].grid()
		pad = (maxx-minx)*0.05
		plotfig.axes[i].set_xlim(minx-pad,maxx+pad)
		if i%2==0:
	...
	for i in range(d2.shape[0]):
		...
		global haserror
		if d2[i,:].shape[1]==3:
			plotfig.axes[i].errorbar(d2[i,:,0],d2[i,:,1]/normfac,d2[i,:,2]/normfac,label='Freq '+str(i+1))
			haserror = True
		else:
			plotfig.axes[i].plot(d2[i,:,0],d2[i,:,1]/normfac,label='Freq '+str(i+1))
			haserror = False

	if d2.shape[0]==1:
		try:
			simplot_i1, = plotfig.axes[0].plot(d2[0,:,0],numpy.zeros(shape=len(d2[0,:,0])),color='red',label=r'Sim')
	...
	if d2.shape[0]==2:
		try:
			...
			if haserror:
				...
				plotfig.axes[2].errorbar(d2[0,:,0],spi1,spi1err,color='green',label=r'$\alpha$ 1-2')
			else:
				plotfig.axes[2].plot(d2[0,:,0],spi1,color='green',label=r'$\alpha$ 1-2')
		except:
			if debug_out:
				traceback.print_exc()
			pass
		...
		try:
			simplot_i1, = plotfig.axes[0].plot(d2[0,:,0],spi1*0,color='red',label=r'Sim')
			simplot_i2, = plotfig.axes[1].plot(d2[0,:,0],spi1*0,color='red',label=r'Sim')
			simplot_a12, = plotfig.axes[2].plot(d2[0,:,0],spi1,color='red',label=r'Sim')
			xlim = plotfig.axes[0].get_xlim()
			ylim = plotfig.axes[0].get_ylim()
			chiplot_1, = plotfig.axes[0].plot(numpy.mean(xlim),numpy.mean(ylim),label=r"$\chi$="+str(0),alpha=0)
	...
	if d2.shape[0]==3:
		try:
			...
			if haserror:
				spi1err = (1/numpy.log(numpy.float(frequency1field.get())/numpy.float(frequency2field.get())))*((d2[0,:,2]/d2[0,:,1])**2+(d2[1,:,2]/d2[1,:,1])**2)**0.5
				plotfig.axes[3].errorbar(d2[0,:,0],spi1,spi1err,color='green',label=r'$\alpha$ 1-2')
			else:
				plotfig.axes[3].plot(d2[0,:,0],spi1,color='green',label=r'$\alpha$ 1-2')
			plotfig.axes[3].legend()
		except:
			if debug_out:
				traceback.print_exc()
			pass
		...
		try:
			...
			if haserror:
				...
				plotfig.axes[4].errorbar(d2[0,:,0],spi2,spi2err,color='green',label=r'$\alpha$ 2-3')
			else:
				plotfig.axes[4].plot(d2[0,:,0],spi2,color='green',label=r'$\alpha$ 2-3')
			plotfig.axes[4].legend()
		except:
			if debug_out:
				traceback.print_exc()
			pass
	...
	if d2.shape[0]==4:
		try:
			...
			if haserror:
				spi1err = (1/numpy.log(numpy.float(frequency1field.get())/numpy.float(frequency2field.get())))*((d2[0,:,2]/d2[0,:,1])**2+(d2[1,:,2]/d2[1,:,1])**2)**0.5
				plotfig.axes[4].errorbar(d2[0,:,0],spi1,spi1err,color='green',label=r'$\alpha$ 1-2')
			else:
				plotfig.axes[4].plot(d2[0,:,0],spi1,color='green',label=r'$\alpha$ 1-2')
			plotfig.axes[4].legend()
		except:
			if debug_out:
				traceback.print_exc()
		...
		try:
			...
			if haserror:
				...
				plotfig.axes[5].plot(d2[0,:,0],spi2,color='green',label=r'$\alpha$ 2-3')
			plotfig.axes[5].legend()
		except:
			if debug_out:	
				traceback.print_exc()
			pass
		...
		try:
			...
			if haserror:
				...
				plotfig.axes[6].plot(d2[0,:,0],spi3,color='green',label=r'$\alpha$ 2-4')
			plotfig.axes[6].legend()
		except:
			if debug_out:
				traceback.print_exc()
			pass
	...
	for i in range(len(plotfig.axes)):
		plotfig.axes[i].legend(prop={'size': 12})
...
#		print round(chi_array[0],4)
	...
	if d2.shape[0]==2:
		...
		chi_array[2] = chisq(spi1,sim_a12)
		plotfig.axes[0].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[0],4)))
		plotfig.axes[1].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[1],4)))
		...
		plotfig.axes[2].legend(handles,labels)
		plotfig.axes[2].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[2],4)))
	...
	if d2.shape[0]==3:
		...
		plotfig.axes[1].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[1],4)))
		plotfig.axes[2].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[2],4)))
		plotfig.axes[3].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[3],4)))
		plotfig.axes[4].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[4],4)))
	...
	if d2.shape[0]==4:
		...
		plotfig.axes[3].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[3],4)))
		plotfig.axes[4].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[4],4)))
		plotfig.axes[5].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[5],4)))
		plotfig.axes[6].legend_.texts[0].set_text(r"$\chi$="+str(round(chi_array[6],4)))

	plotcanvas.draw()

def run_noplot(varstochange,vals):
	global filenames, def_mode, def_v0, def_cri, def_b0, def_b1, def_hb1, def_hb2, def_D0, def_gmode, def_Z1, def_hv, def_mode, def_ablosses, def_mu, def_velfield, def_norm, def_epsilon, data, chi_array
...
#		print varstochange[v],vals[v]
		exec("loc_{0}={1}".format(varstochange[v],vals[v]),globals())

#	print def_cri,loc_def_cri
...
def search():
	...
	for i in range(len(currentvalues)):
		if searchres.get()=='very coarse':
			searches.append((pararray[i],searchdict2[pararray[i]][0],searchdict2[pararray[i]][1],searchdict2[pararray[i]][2]))

		if searchres.get()=='coarse':
			searches.append((pararray[i],searchdict[pararray[i]][0],searchdict[pararray[i]][1],searchdict[pararray[i]][2]))
		...
		if searchres.get()=='medium':
			...
			searches.append((pararray[i],minval,maxval,stepsize))
...
#	print orderedresult[0]
#	print orderedresult[1]
#	print orderedresult[2]
#	print float(orderedresult[0][0][1:-1].split(',')[0]),float(orderedresult[0][0][1:-1].split(',')[1])
#	print float(orderedresult[1][0][1:-1].split(',')[0]),float(orderedresult[1][0][1:-1].split(',')[1])
#	print float(orderedresult[2][0][1:-1].split(',')[0]),float(orderedresult[2][0][1:-1].split(',')[1])
...
def apply_vals():
	...
	pararray = ['def_cri', 'def_b0', 'def_b1', 'def_hb1', 'def_hb2', 'def_Z1', 'def_v0', 'def_hv', 'def_D0', 'def_mu']
	labelarray = ['def_cri', 'def_b0', 'def_b1', 'def_hb1', 'def_hb2', 'def_Z1', 'def_v0', 'def_hv', 'def_D0', 'def_mu']
#	global tosearch,best_results_array,filenames, def_mode, def_v0, def_cri, def_b0, def_b1, def_hb1, def_hb2, def_D0, def_gmode, def_Z1, def_hv, def_mode, def_ablosses, def_mu, def_velfield, def_norm, def_epsilon, data, chi_array
	for i in range(len(tosearch)):
		exec('{0}={1}'.format(tosearch[i],best_results_array[i],globals()))
#		exec('{0}={1}'.format("tosearch["+str(i)+"]","best_results_array"+"["+str(i)+"]",globals()))
#		exec('{0}={1}'.format(tosearch[i],"best_results_array"+"["+str(i)+"]",globals()))
#		exec('{0}={1}'.format(tosearch[i],best_results_array[i],globals()))
	global applyvalues
	applyvalues=1
	initial_plot()
...
#	Tk.Label(master=parsearchframe,textvariable=bestresultchi,text='0').grid(row=16,column=1,sticky=Tk.W)
#	Tk.Label(master=parsearchframe,text='Best Combination:').grid(row=17,column=0,sticky=Tk.W)
#	Tk.Label(master=parsearchframe,textvariable=bestresult).grid(row=17,column=1,sticky=Tk.W)
...
#parsearch_button = Tk.Button(configframe, text='Parameter Search', command=open_parsearch_window)
